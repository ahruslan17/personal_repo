...
class TradingTaskBybit:
    ...
    async def getOrderBook(self):
        while True:
            ...
            current = {'best_bid_0': ob['bids'][0], 'best_bid_15': ob['bids'][15], 'best_ask_0': ob['asks'][0]}
        ...
        while True:
            ...
            for i in range(len(p)):
                if p[i]['info']['symbol'] == self.ticker and p[i]['info']['side'] == 'Buy' and flag == 'Enter':
                    print('НОВАЯ ПОЗИЦИЯ НА ПОКУПКУ ОТКРЫТА!!!!!')
    ...
    def FuturesExitOrderLimit(self):
        # global ticker, qActiveOrders, qActivePositions, isSold
        s_time = time.time()
        print('Количество активов для закрытия позции: ', self.pos_size_to_sell)
        response = self.session_bybit.place_order(
    ...
    async def main_logic(self):
        ...
        # self.session_bybit = HTTP(api_key='', api_secret='')
        ...
        try:
                    ...
                    if self.cum_margin >= self.ALL_MARGIN - 1:  # полностью вошли в позицию
                        ...
                        if time.time() * 1000 >= self.pos_start_datetime + self.TIMEOUT * 60 * 1000:
                            ...
                            if self.qActivePositions == 1:
                                self.FuturesExitOrderLimit()

                            # как только позиция закроется, isSold->True и дальше сигнал на оконччание торгового задания
                            while not self.isSold:
                                positions_task = asyncio.create_task(self.getPositions(flag='Exit'))
                        ...
                        if self.qActiveOrders == 0:
                            ...
                            order_code, order_id = self.FuturesEnterOrder(self.ALL_MARGIN - self.cum_margin)
                            if order_code == 0:
            ...
            print('ОШИБКА ', ex)
            asyncio.create_task(
                self.telegram_bot.sendMessageDebug('TradingTaskClass_bybit_f main error'))
            await self.exchange.close()
        finally:
            # orders_task.cancel()
            await self.exchange.close()
    ...
    def getNewGrowthPercentage(self):
        ...
        candles = exchange.fetch_ohlcv(self.ticker, '1m', since=start_timestamp, limit=10)
        print(len(candles))
    ...
    def main(self):
        ...
        self.changeMarginModeAndLeverageBybit()
