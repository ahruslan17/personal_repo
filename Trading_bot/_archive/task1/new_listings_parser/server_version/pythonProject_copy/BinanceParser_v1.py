...
# TODO: Большая часть этой функции повторяет код предыдущей функции. Думаю, стоит их как-то объединить,
# чтобы не было много копипасты
def parse_announcement_table_futures(url):
    ...
    for key, value in data_dict.items():
        ...
        cleaned_value = value[0].replace('\xa0', ' ') # TODO Зачем мы храним полный список value, если тут берем только первый элемент?
...
def parse_announcement_table_spot(url):
    ...
    if response.status_code == 200:
        ...
        # TODO. Мне кажется, здесь ошибка в логике. Если стейбл залистят на споте, то мы попадем в первое условие
        # "if is_new_spot_pair", а во второе "elif is_stable_coin" - нет, и не узнаем, что это был стейбл на самом деле.
        elif is_stable_coin:
...
# TODO. Хотелось бы оптимизировать данную функцию. Во-первых, очень много повторяющегося кода внутри.
# Во-вторых, поиск новых статей через сравнение с полным списком предыдущих кажется не очень оптимальным.
# Можно, например, сравнивать новую статью с последней обработанной или как-то по индексам.
# И, конечно, когда разберешься с базами данных, нужно будет избавиться от хард-кода предыдущих статей.
...
# TODO. Может быть тупой вопрос, но почему парсес остановится, если мы вызовем команду stop из ТГ?
# У нас же при вызове команды start запускается функция, которая крутится в бесконечном лупе.
...
async def run_existing_code():
    ...
    while True:
        ...
        time.sleep(5) # TODO Думаю, надо уменьшить задержку, чтобы получать новости как можно быстрее. Или тогда rate limits будем ловить?
