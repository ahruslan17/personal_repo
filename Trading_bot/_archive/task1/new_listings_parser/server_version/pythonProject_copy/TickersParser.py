...
# TODO. Парсеры для разных бирж содержат много повторящегося кода. Возможно, лучше вынести парсинг сырых данных по API 
# в одну отдельную функцию, и для каждой биржи оставить только ту часть функици, которая обрабатывает полученные сырые данные.
# А вообще, лучше перейти на ccxt и делать все через него.
...
def bybit_request_token():
    ...
    try:
        ...
        asyncio.run(send_telegram_message_debug(f'Парсер бирж Error. Возникла ошибка: {e}'))
...
def mexc_request_token():
    ...
    try:
        ...
        print(f"Произошла ошибка при обращении к futures: {e}")
        asyncio.run(send_telegram_message_debug(f'Возникла ошибка: {e}'))
        logging.info(f"Произошла ошибка: {e}")
...
def main():
    try:
        ...
        asyncio.run(
            send_telegram_message_debug('Парсер бирж на предмет наличия токенов запущен. Обновление базы каждый час.'))
        ...
        while True:
            ...
            if i > 0:
                asyncio.run(send_telegram_message_debug(f"{formatted_time} (UTC+3) - Обновление базы \n База данных успешно сохранена в файл: {file_path}"))

            time.sleep(3600)
    except KeyboardInterrupt:
        # Обработка события прерывания (нажатие кнопки "Stop" в PyCharm)
        asyncio.run(send_telegram_message_debug("Парсер тикеров на биржах остановлен вручную"))
