...
def getFuturesSize_okx(ticker):
    ...
    try:
        ...
        print('getFuturesSize_okx Error', e)
        logging.info(f"getFuturesSize_okx Error: {e}")
        return -11
...
def FuturesEnterOrder_okx(ticker, position_sum, leverage):
    ...
    try:
        ...
        print('----')
        print(getFuturesPrice_okx(ticker))
        print(pos_amount)
        print(float(getNumberAfterDot(ticker)))
        print(round(float(pos_amount), int(getNumberAfterDot(ticker))))
        print(round(float(pos_amount), int(getNumberAfterDot(ticker)) - 1))
        result = tradeAPI.place_order(
            instId=ticker.replace('USDT', '-USDT') + '-SWAP',
            tdMode='isolated',
            side='buy',
...
# print(FuturesEnterOrder_okx('AVAXUSDT',10,2))
